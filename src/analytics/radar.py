...
class RadarChartEngine:
    ...
    def load(self):
        ...
        return ratios.merge(peers, on="company_id", how="left")
    ...
    def plot_company(self, company):
        ...
        if pd.isna(peer):
            ...
            peer_avg = df[df["peer_group_name"] == peer][self.METRICS].mean()
        ...
        peer_avg = np.concatenate([peer_avg, [peer_avg.iloc[0]]])
        ...
        ax.plot(angles, peer_avg, linestyle="--", linewidth=2, label="Peer Average")

        ax.set_xticks(angles[:-1])

        ax.set_xticklabels(labels, fontsize=10)

        ax.set_title(f"{company} Radar", fontsize=14)

        ax.legend(loc="upper right")

        plt.tight_layout()

        plt.savefig(f"reports/radar_charts/{company}_radar.png", dpi=300)
