...
with pd.ExcelWriter(OUTPUT, engine="openpyxl") as writer:
    ...
    for group in groups["peer_group_name"].dropna().unique():
        ...
        group_companies = groups[groups["peer_group_name"] == group]["company_id"]

        df = peer[peer["company_id"].isin(group_companies)]

        values = df.pivot_table(
            index=["company_id", "year"], columns="metric", values="value"
        )
        ...
        final = final.merge(companies, on="company_id", how="left")

        cols = ["company_id", "company_name", "year"] + [
            c for c in final.columns if c not in ["company_id", "company_name", "year"]
        ]
        ...
        summary = {col: "" for col in final.columns}
        ...
        final = pd.concat([final, pd.DataFrame([summary])], ignore_index=True)
