...
def populate():
    ...
    merged = merged.merge(cf, on=["company_id", "year"], how="left")
    ...
    for company in merged["company_id"].unique():
        ...
        for _, row in company_df.iterrows():
            ...
            npm = RatioEngine.net_profit_margin(row["net_profit"], row["sales"])
            ...
            revenue5, _ = revenue_cagr_5yr(company_df["sales"].tolist())

            pat5, _ = pat_cagr_5yr(company_df["net_profit"].tolist())

            eps5, _ = eps_cagr_5yr(company_df["eps"].tolist())
